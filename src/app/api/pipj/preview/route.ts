...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const colab of colaboradores) {
      ...
      // sendo pré-visualizado — regra da quinzena: projeto iniciado a partir
      // do dia 16 só conta a partir do mês seguinte (ver
      // src/lib/pipj-projetos.ts). Sem sincronização do Monday ainda
      // (detalhe nulo), cai no histórico de auditoria antigo, e na falta
      // dele, no valor corrente.
      ...
      const detalhesCalculo = {
        ...
        // resolvePipjCargoKey. Só para depuração/auditoria; a coluna "cargo"
        // abaixo continua mostrando o cargo real do colaborador.
