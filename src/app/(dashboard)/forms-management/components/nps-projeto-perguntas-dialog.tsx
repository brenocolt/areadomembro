...
// edição direta.
type Edicao = Record<string, { titulo: string, criterios: Record<number, string>, obs?: string }>
...
function BlocoEditor({ titulo, icon, edicao, setCampo }: {
    ...
    edicao: Edicao
    setCampo: (campo: string, patch: Partial<{ titulo: string, criterios: Record<number, string> }>) => void
}) {
    return (
                    ...
                    <div className="space-y-1.5">
                        ...
                        {[1, 2, 3, 4, 5].map(v => (
                            <div key={v} className="flex items-start gap-2">
                                ...
                                <Textarea
                                    value={d.criterios[v] || ''}
                                    onChange={(e) => setCampo(campo, { criterios: { ...d.criterios, [v]: e.target.value } })}
...
export function NpsProjetoPerguntasDialog({ open, onOpenChange }: Props) {
    ...
    const setCampoGerente = (campo: string, patch: Partial<{ titulo: string, criterios: Record<number, string> }>) =>
        setGerente(prev => ({ ...prev, [campo]: { ...prev[campo], ...patch } }))
    const setCampoConsultor = (campo: string, patch: Partial<{ titulo: string, criterios: Record<number, string> }>) =>
