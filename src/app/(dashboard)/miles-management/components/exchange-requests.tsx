...
export function ExchangeRequests() {
    ...
    const handleAccept = async (req: any) => {
        ...
        if (!error) {
            ...
            if (req.item === '-1 Ponto') {
                const { data: colData } = await supabase.from('colaboradores').select('pontos_negativos').eq('id', req.colaborador_id).single();
                if (colData) {
                    const newPoints = Math.max(0, (colData.pontos_negativos || 0) - 1);
                    await supabase.from('colaboradores').update({ pontos_negativos: newPoints }).eq('id', req.colaborador_id);

                    await supabase.from('solicitacoes_remocao').insert({
                        colaborador_id: req.colaborador_id,
                        motivo: 'Resgate de Recompensa: -1 Ponto (10 Milhas)',
                        pontos_solicitados: 1,
                        status: 'APROVADA'
                    });
                }
            }
    ...
    const handleReject = async (req: any) => {
        const { error } = await supabase.from('milhas_trocas').update({ status: 'REPROVADA' }).eq('id', req.id);
        if (!error) {
            setRequests(prev => prev.filter(r => r.id !== req.id));
            window.dispatchEvent(new Event('refreshMilesData'))
        } else {
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-slate-100 dark:border-white/5">
                <div className="flex items-center gap-3">
                    <div className="bg-cyan-50 dark:bg-cyan-500/10 p-2 rounded-xl border border-cyan-100 dark:border-cyan-500/20">
                        <ShoppingBag className="h-5 w-5 text-cyan-600 dark:text-cyan-500" />
                    </div>
                    <CardTitle className="text-base font-bold text-slate-900 dark:text-white">Pedidos de Troca Pendentes</CardTitle>
                </div>
                            ...
                            {requests.map((req) => (
                                <div key={req.id} className="grid grid-cols-[1.5fr_2fr_1fr_1fr_0.5fr] gap-4 items-center p-3 hover:bg-slate-50 dark:hover:bg-white/5 rounded-xl transition-all group border border-transparent hover:border-slate-100 dark:hover:border-white/5">
                                    ...
                                    <div className="text-xs font-bold text-slate-900 dark:text-white">
                                        <span className="text-cyan-600 dark:text-cyan-400">{req.miles.toLocaleString('pt-BR')}</span> <span className="text-cyan-600/50 dark:text-cyan-400/50">milhas</span>
                                    </div>
                        ...
                        {requests.map((req) => (
                            <div key={req.id} className="bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 p-4 rounded-3xl flex flex-col gap-3 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors group">
                                <div className="flex justify-between items-start">
                                    ...
                                    <div className="bg-cyan-50 dark:bg-cyan-500/10 border border-cyan-100 dark:border-cyan-500/20 px-2 py-1 rounded-lg text-[10px] font-bold text-cyan-600 dark:text-cyan-400">
                                ...
                                <div className="bg-white/50 dark:bg-black/20 p-3 rounded-xl border border-slate-100 dark:border-white/5 mt-2">
                                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300">{req.item}</p>
                                </div>
